...
import (
	...
	bn "github.com/wonderstone/GEP-MOD/functions/bool_nodes"
	in "github.com/wonderstone/GEP-MOD/functions/int_nodes"
	mn "github.com/wonderstone/GEP-MOD/functions/math_nodes"
...
func (g *Generation) Evolve(iterations int, expectFitness float64, pm float64, pis float64, glis int, pris float64, glris int, pgene float64, p1p float64, p2p float64, pr float64) *genome.Genome {
...
// linkFunc is the linking function used to combine the genes within a genome.
// sf is the scoring (or fitness) function.
func NewGS(fs []gene.FuncWeight, funcType functions.FuncType, numGenomeSets, headSize, numGenomesPerGenomeSet, numGenesPerGenome, numTerminals, numConstants int, linkFunc string, sf genomeset.ScoringFunc) *GenerationGS {
	r := &GenerationGS{
...
func (gs *GenerationGS) EvolveGS(iterations int, expectFitness float64, pm float64, pis float64, glis int, pris float64, glris int, pgene float64, p1p float64, p2p float64, pr float64) *genomeset.GenomeSet {
	// Algorithm flow diagram, figure 3.1, book page 56
	for i := 0; i < iterations; i++ {
...
func (gs *GenerationGS) replication() {
	...
	for i := 0; i < len(gs.GenomeSets); i++ {
		beta += rand.Float64() * 2.0 * maxWeight
		for beta > gs.GenomeSets[index].Score {
			beta -= gs.GenomeSets[index].Score
